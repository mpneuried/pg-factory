...
module.exports = ( options, escape = SqlString.escape )->
	...
	return class SQLBuilder extends require( "./basic" )
		...
		###
		initialize: =>
			@define( "schema", @getSchema, @setSchema )
		...
		update: ( attributes, options = {} )=>
			...
			statement = []
			statement.push "UPDATE #{ @schema }.#{ @table }"
				...
				if pred is null
					...
					switch _operand
						...
						when "sub"
							...
							subtable.schema = _val.schema or @schema
	...
	# # Getter and Setter methods
		...
		getJoins: =>
			_joins = []
			for _jn in @_c.joins
				_joins.push "#{_jn.type.toUpperCase()} JOIN #{_jn.schema}.#{_jn.table} ON #{@table}.#{_jn.field} = #{_jn.table}.#{_jn.foreignField}"
			return _joins.join( "\n" )
					...
					switch _cnf.type
						...
						when "number", "N"
							...
							if not _val?
								...
								_vals.push( "extract(epoch from now() )*1000" )
							else if _val is "incr"
								_vals.push( "(CASE WHEN #{ _key } is NULL THEN 0 ELSE #{ _key } + 1 END )" )
						...
						when "boolean", "B"
							if _cnf.booleanString
								_bVals = [ 0, 1 ]
							else
								_bVals = [ false, true ]
							switch _val.toString().toLowerCase()
								when "true","1", "y","yes" then _vals.push( _bVals[1] )
								else _vals.push( _bVals[0] )
							_keys.push( _key )
						...
						when "unixtimestamp", "U"
							if _val is "now"
								_vals.push( "extract(epoch from now() )" )
							else if _isDate( _val )
